...
- `SIP_STT_MAKEUP_DB` — makeup gain (dB) added to the caller signal the transcriber
  sees, to recover the quiet speech a caller produces over the bot (the bridge's echo
  canceller is not the cause; the rationale is in `stt.py`). Default **0** = off; **6**
  recovered the quiet barge-in "stop"s the engine was dropping, with no regressions on
  205 clips. SIP only; VAD and endpointing are upstream of it and unaffected. The
  OpenClaw path does its own echo cancellation and never sees this even though it
  shares this file.
