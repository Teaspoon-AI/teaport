...
# A sentence the completion emits more than once. This is a DIFFERENT failure from
# the punctuation collapse above and needs its own detector, because the text is
# otherwise perfectly well-formed — nothing in it looks degenerate.
#
...
# spoke 13.5s of audio for a one-sentence reply. Then live 2026-08-26 08:48 the same
# thing happened NEAR-verbatim: one completion, and the deltas carried the whole reply
# twice at generation pace (copy one 08:48:13.0-15.6, a 1.0s gap, copy two
# 08:48:16.6-19.1), with the numbers re-sampled on the second pass ("four thousand five
# hundred eight" vs "four five zero eight") — which a verbatim comparison missed
# entirely. The re-sampled copies are also what settles WHERE the doubling happens:
# no downstream frame replay can re-word a number, so the duplicate is generated by
# the model and arrives inside the provider's stream. The fuzzy match and its
# thresholds live in tts_text (MIN_REPEAT_CHARS / REPEAT_SIMILARITY /
# is_sentence_repeat), shared with llm_text_guard's repeat cut so the log and the
# guard cannot disagree about the same completion.


def _repeated_sentence(text: str) -> str:
    """The first sentence that near-repeats an earlier one, or ""."""
    seen: list[str] = []
    for sentence in SENTENCE_SPLIT.split(text):
        sentence = sentence.strip()
        if len(sentence) < MIN_REPEAT_CHARS:
            continue
        if any(is_sentence_repeat(prior, sentence) for prior in seen):
            return sentence
