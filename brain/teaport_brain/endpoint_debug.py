...
class EndpointDebug(FrameProcessor):
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        m = self._m
        t = time.monotonic()
        # Stage-independent: Smart Turn's verdict rides a MetricsFrame that the user
        # aggregator queues DOWNSTREAM of itself, and the aggregator sits below the
        # "in" tap — so the "in" tap can never see it, and for a year every TURN-COMMIT
        # line claimed "no verdict" no matter what the model said. Record it wherever
        # this tap happens to be.
        ...
        if self._stage == "in":
            if isinstance(frame, VADUserStartedSpeakingFrame):
                ...
                if p is not None:
                    ...
                    ver = " · SmartTurn verdict not seen yet"
